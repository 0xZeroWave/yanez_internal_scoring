...
def calculate_variation_scores(data: dict, variation_config: dict) -> dict:
        ...
        """
        from app.service.reward import calculate_variation_quality, get_name_variation_rewards
        filtered_data = {}
        scores_data = {}
        final_scores = []
        ...
        for name, variations in data.items():
            ...
            if variations:
                try:
                    ...
                    continue
        print(f"Average final score across all names: {np.mean(final_scores) if final_scores else 0:.3f}")
        return {
